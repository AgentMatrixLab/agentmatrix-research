...
`AGENTS.md` is a lightweight repository entry that lists existing modules,
points to the existing CLI entry points, names the bundled Skills and the
most relevant `docs/` files, and records practical environment limits and
safety rules. This README intentionally does not duplicate that navigation
guide.
