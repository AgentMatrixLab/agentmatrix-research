...
## Agent API Capability Reference

The following 13 self-documenting functions (manifest is the single source of truth at
`research_core/agent_manifest.py`) share a unified Python + CLI entry point. Field names and
required/optional parameters match exactly between Manifest, Python API, CLI, and `AGENTS.md`.

| You want to... | Python | CLI | Key returns |
|---|---|---|---|
| See all capabilities | `discover()` | `discover` | `framework, version, capabilities[]` |
| Framework overview | `overview()` | `overview` | `workspace, factor_families, data_sources, paths` |
| Check data source | `check_data_source(env_file="")` | `check-data` | `connected (bool), details, next_actions` |
| Explore factors | `explore_factors(goal, universe, factor_set="alpha101", start="", end="", top_n=10, horizon=5, output_dir="", cache_dir="")` | `explore --goal ... --universe ... [--horizon 5] [--output-dir ""] [--output result.json]` | `gate_verdict, top_factors, summary, report_path, artifacts{job_path}, next_actions` |
| Validate a factor | `validate_factor(factor_name, ic_mean=..., ic_ir=..., oos_retention=..., turnover=...)` | `validate --factor-name ... --ic-mean ... --ic-ir ... --oos-retention ...` | `passed (bool), gates{}, fail_reasons[], pass_reasons[]` |
| Evaluate factor CSV | `evaluate_factor_csv(factor_csv, factor_name, returns_csv="", horizon=5)` | `evaluate-csv --factor-csv ... --factor-name ...` | `status, mean_rank_ic, rank_icir, turnover, warnings[]` |
| List available factors | `list_factors(factor_set="alpha101")` | `list-factors [--factor-set alpha101]` | `factor_set, total, factors[]{name, implemented, proof_status}` |
| Build strategy signals | `build_strategy(validated_run_path, top_n=50, rebalance_frequency="daily", long_only=True)` | `build --validated-run ... [--top-n 50] [--rebalance-frequency daily]` | `strategy_id, artifacts{signals, config}` |
| Package for backtest | `package_backtest(engine, signal_path, start="", end="", metadata_path="")` | `package --engine gm --signal-path ... [--start ""] [--end ""]` | `run_id, engine, package_dir, package_path, artifacts{}` |
| Parse backtest result | `parse_backtest_result(engine, run_id, result_path)` | `parse-result --engine ... --run-id ... --result-path ...` | `run_id, engine, status, metrics{}, source_path, artifacts{}, diagnostics[]` |
| Mine a factor (Qlib) | `mine_factor(name, expression, description="", start="", end="", universe="csi300")` | `mine --name ... --expression ...` | `factor_name, ic_mean, ic_ir, rank_ic_mean, long_short_spread, definition, evaluation, top_metrics, status` |
| Auto-mine factors (AI) | `auto_mine(theme, start="", end="", universe="csi300", candidate_limit=10)` | `auto-mine --theme ...` | `theme, generated_count, results[], candidates[], best_factor, best_ic` |
| Qlib expression backtest | `qlib_backtest(factor_expression, start="", end="", universe="csi300", benchmark="SH000300")` | `qlib-backtest --factor-expression ...` | `expression, annualized_return, sharpe_ratio, max_drawdown, total_return, volatility, win_rate, metrics{}, equity_curve` |

Factor families and their canonical sizes: Alpha101=101, WQ101=10, GTJA191=191, Alpha158=158, Barra=10.
