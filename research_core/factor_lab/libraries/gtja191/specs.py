...
GTJA191_SOURCE = "本文档附录 A/B 内置公式源；源自国泰君安《基于短周期价量特征的多因子选股体系》表 6 / 附录 2，经 gtja191_formula_manifest 清洗"
GTJA191_VERSION = "v2026.06-paper-formula-manifest"
GTJA191_FORMULAS: dict[int, str] = {
    ...
    21: 'REGBETA(MEAN(CLOSE,6),SEQUENCE(6))',
    ...
    28: '3*SMA((CLOSE-TSMIN(LOW,9))/(TSMAX(HIGH,9)-TSMIN(LOW,9))*100,3,1)-2*SMA(SMA((CLOSE-TSMIN(LOW,9))/(MAX(HIGH,9)-TSMAX(LOW,9))*100,3,1),3,1)',
    29: '(CLOSE-DELAY(CLOSE,6))/DELAY(CLOSE,6)*VOLUME',
    30: 'WMA((REGRESSION_RESIDUAL(CLOSE/DELAY(CLOSE,1)-1,MKT,60))^2,20)',
    ...
    54: '(-1* RANK((STD(ABS(CLOSE - OPEN)) + (CLOSE - OPEN)) + CORR(CLOSE, OPEN,10)))',
    ...
    111: 'SMA(VOL*((CLOSE-LOW)-(HIGH-CLOSE))/(HIGH-LOW),11,2)-SMA(VOL*((CLOSE-LOW)-(HIGH-CLOSE))/(HIGH-LOW),4,2)',
    ...
    127: '(MEAN((100*(CLOSE-MAX(CLOSE,12))/(MAX(CLOSE,12)))^2))^(1/2)',
    ...
    143: 'RECURSIVE_RETURN_CHAIN(CLOSE,DELAY(CLOSE,1))',
    ...
    149: 'REGBETA(FILTER(CLOSE/DELAY(CLOSE,1)-1,BENCHMARKINDEXCLOSE<DELAY(BENCHMARKINDEXCLOSE,1)),FILTER(BENCHMARKINDEXCLOSE/DELAY(BENCHMARKINDEXCLOSE,1)-1,BENCHMARKINDEXCLOSE<DELAY(BENCHMARKINDEXCLOSE,1)),252)',
    ...
    165: 'TSMAX(SUM(CLOSE-MEAN(CLOSE,48),48),48)-TSMIN(SUM(CLOSE-MEAN(CLOSE,48),48),48)/STD(CLOSE,48)',
    166: '-20*(20-1)^1.5*SUM(CLOSE/DELAY(CLOSE,1)-1-MEAN(CLOSE/DELAY(CLOSE,1)-1,20),20)/((20-1)*(20-2)*(SUM(CLOSE/DELAY(CLOSE,1),20)^2))^1.5',
    ...
    183: 'TSMAX(SUM(CLOSE-MEAN(CLOSE,24),24),24)-TSMIN(SUM(CLOSE-MEAN(CLOSE,24),24),24)/STD(CLOSE,24)',
