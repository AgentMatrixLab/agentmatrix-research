...
def pairwise_correlation(
    ...
    Args:
        ...
        start, end: date range (used when values_df is None)
        values_df: optional pre-computed factor values DataFrame with
            one column per factor; if provided, bypasses Qlib data loading

    Returns:
        Correlation matrix (DataFrame, indexed by factor_id)
    """
    if values_df is not None:
        return values_df[factor_ids].corr()
...
def find_redundant(
    ...
    Args:
        ...
        values_df: optional pre-computed DataFrame; bypasses Qlib
