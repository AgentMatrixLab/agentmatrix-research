...
def compute_monthly_ic(factor_df, returns_df, date_col='date', code_col='code'):
    """
    计算月频IC序列
    
    参数:
        factor_df: DataFrame, 因子值 [date, code, alpha...]
        returns_df: DataFrame, 收益率 [date, code, return]
    ...
    for date in merged[date_col].unique():
        ...
        for alpha_col in alpha_cols:
            day_ic[alpha_col] = compute_ic(day_data[alpha_col], day_data['return'])
