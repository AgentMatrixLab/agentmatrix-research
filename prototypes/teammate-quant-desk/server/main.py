"""
QUANT DESK 后端 API — FastAPI + Backtrader 引擎
...
def list_strategies():
    ...
    for r in rows:
        nav_row = db2.execute(
            "SELECT nav FROM nav_series WHERE strategy_id=? ORDER BY date DESC LIMIT 2", [r["id"]]
        ).fetchall()
        today_ret = 0.0
        annual_ret = 0.0
        sharpe = 0.0
        ...
        # 拉 KPI
        kpi = db2.execute("SELECT annual_return, sharpe FROM kpis WHERE strategy_id=?", [r["id"]]).fetchone()
        if kpi:
...
def _get_sub_strategies(strategy_id: str) -> list:
    ...
    if strategy_id == "div_v6":
        return [
            {"name": "高股息龙头", "weight": 0.28, "contribution": 0.12},
            {"name": "红利低波", "weight": 0.22, "contribution": 0.08},
            {"name": "红利质量", "weight": 0.20, "contribution": 0.06},
            {"name": "红利成长", "weight": 0.18, "contribution": 0.04},
            {"name": "红利动量", "weight": 0.12, "contribution": 0.02},
        ]
    return []
...
def get_positions(strategy_id: str = Query("alpha")):
    db = get_db()
    rows = db.execute(
        "SELECT * FROM holdings WHERE strategy_id=? ORDER BY value DESC", [strategy_id]
    ).fetchall()
