...
def run(source: str, params: Params, on_progress: Optional[Callable] = None) -> EngineResult:
    ...
    内存流:
      1. 按日期范围切片 kline（不加载全量）
      2. 选 MAX_SYMBOLS 只股票做 data feed（不把全市场加载）
      3. cerebro.run(runonce=False, preload=False) — 不预分配
      4. 跑完后 del cerebro → gc.collect()
    """
    ...
    for sym in symbols:
        ...
        df = df.fillna(method="ffill")  # 剩余的 NaN 继续前向填
    ...
    local_vars = {}
    exec(compile(source, "<strategy>", "exec"), restricted_globals, local_vars)
...
def _select_symbols(kline, symbols, max_n):
    """按数据完整度排序选前 N 只"""
    counts = kline.groupby("symbol").size()
    available = sorted(counts.index.tolist(), key=lambda s: -counts[s])
