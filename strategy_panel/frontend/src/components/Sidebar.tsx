...
import { listStrategies, type ApiStrategySummary } from "@/api";
...
function Spark({ data, tone }: { data: number[]; tone: "up" | "down" }) {
  if (!data || data.length < 2) return <span className="h-[22px] w-[60px] shrink-0" />;
  const w = 60;
  const h = 22;
...
export default function Sidebar() {
  ...
  useEffect(() => {
    listStrategies()
      .then((list) => {
        // 按年化收益降序取前 5
        const sorted = list.sort((a, b) => b.annualReturn - a.annualReturn).slice(0, TOP_N);
        setStrategies(sorted);
        // 默认选中收益最高的
        if (sorted.length > 0 && !sorted.find((s) => s.id === strategyId)) {
          setStrategy(sorted[0].id);
        }
        setLoaded(true);
      })
      .catch(() => setLoaded(true)); // 静默失败
  }, []); // 只在挂载时加载一次
