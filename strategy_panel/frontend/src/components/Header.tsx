...
export default function Header() {
  const { def, bench, points } = useDashboard();
  ...
  const dateFrom = points.length > 0 ? points[0].date : "加载中";
  const dateTo = points.length > 0 ? points[points.length - 1].date : "...";
        ...
        {isHome ? (
          ...
          <div className="min-w-0">
            ...
            <p className="mt-0.5 text-[11px] text-fg-mute">
              回测区间 {dateFrom} ~ {dateTo} · 基准 {bench.name}
            </p>
          </div>
        ) : (
          /* 其他页面：页面标题 */
