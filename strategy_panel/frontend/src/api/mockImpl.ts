...
export async function listStrategies(): Promise<ApiStrategySummary[]> {
  ...
  return STRATEGIES.map((def) => {
    ...
    const sdata = strategyDataMap[def.id];
    return {
      id: def.id,
      name: def.name,
      tag: def.tag,
...
export async function getStrategyDetail(
  ...
  const win = computeWindow(def.id, benchmark, period);
  const sdata = strategyDataMap[def.id];
  // 从 mock 的字符串 KPI 中提取数值
  const extractPct = (k: any) => {
    const v = parseFloat(String(k.value).replace("%", "")) / 100;
    return isNaN(v) ? 0 : v;
  };
  const kpiArr = win.kpis;
  const nkpis: ApiKpis = {
    totalReturn: extractPct(kpiArr[0] ?? 0),
    annualReturn: extractPct(kpiArr[1] ?? 0),
    sharpe: parseFloat(String(kpiArr[2]?.value ?? "0").replace("%", "")) || 0,
    maxDrawdown: extractPct(kpiArr[3] ?? 0),
    winRate: extractPct(kpiArr[4] ?? 0),
    volatility: extractPct(kpiArr[5] ?? 0),
  };
  return {
    id: def.id,
    name: def.name,
    ...
    trades: sdata.trades,
    subStrategies: (sdata.subStrategies ?? []).map((s) => ({
      id: s.name,
      name: s.name,
      weight: s.weight,
    })),
