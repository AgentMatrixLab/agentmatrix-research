...
| 2026-08-07 | Start smoke service through `${STRATEGY_ROOT}/current` | The prior `set -e` test failure correctly stopped before creating the release symlink | Create the symlink explicitly after the corrected tests are present, then rerun validation |
